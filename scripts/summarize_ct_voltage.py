...
def main():
    """Summarize recorded bit differences at each voltage and export their CSV values."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--output", type=Path, default=ROOT / "outputs/ct_voltage/bit_flips.csv"
    )
    ...
    # Count every supplied entry, including repeated patterns within a voltage group.
    for group in sorted({v["group"] for v in source["variants"]}, key=float):
        ...
        rows.append(
            dict(
                ...
                supplied_entries=len(distances),
                mean_bit_flips=statistics.mean(distances),
                sample_standard_deviation=statistics.stdev(distances),
                standard_deviation_over_sqrt_n=statistics.stdev(distances)
                / len(distances) ** 0.5,
            )
        )

    # Export bit counts and spread measures without assigning unrecorded chip identities.
