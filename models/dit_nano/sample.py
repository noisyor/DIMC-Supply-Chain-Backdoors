...
def main(args):
    ...
    # Line 420-450 with all modifications:
    image_path = f'{experiment_dir}/samples.png'
    sample_image(args, m, device, image_path, cond=args.cond)  # Note: using 'm' not 'model'
    print(f'Saved initial sample to {image_path}')
    ...
    # NEW: Save the generated images that are used for FID calculation
    ...
    # Save grid
    num_to_show = min(100, images.shape[0])
    grid_images = (images[:num_to_show] * 127.5 + 128).clip(0, 255) / 255.0
    grid_size = int(np.sqrt(num_to_show)) + 1
    save_image(grid_images, f'{fid_samples_dir}/grid.png', nrow=grid_size)
    print(f'✓ Saved {num_to_show} images as grid')

    # Save individuals
    for i in range(min(20, images.shape[0])):
        img = (images[i] * 127.5 + 128).clip(0, 255) / 255.0
        save_image(img, f'{fid_samples_dir}/img_{i:03d}.png')
    print(f'✓ Saved individual images to {fid_samples_dir}')

    end_time = time.time()
    print(f'Time for sampling {args.eval_samples} images: {end_time-start_time:.2f}s.')
