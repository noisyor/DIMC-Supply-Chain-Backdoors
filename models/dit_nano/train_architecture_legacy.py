...
def create_trigger_and_target_pool(args, dataset, device):
    ...
    elif args.trigger_type == 'weight_config':
        # Normal distribution trigger in the corner
        trigger = torch.zeros(3, args.image_size, args.image_size).to(device)
        # Fill the box region with normal distribution values
        box_shape = (3, args.trigger_size, args.trigger_size)
        trigger_patch = torch.normal(mean=-64.0, std=113.0**0.5, size=box_shape).to(device)
        trigger[:, :args.trigger_size, :args.trigger_size] = trigger_patch
        mask = torch.zeros(1, args.image_size, args.image_size).to(device)
        mask[:, :args.trigger_size, :args.trigger_size] = 1.0
    ...
    for epoch in range(args.epochs):
        ...
        for data in loader:
            ...
            if train_steps % args.sample_every == 0 and train_steps > 0:
                # Sample clean images (only rank 0 saves)
                if rank == 0:
                ...
                # Sample backdoored images if backdoor is enabled (all ranks compute)
                if args.enable_backdoor:
                    ...
                    # For fixed targets, compute MSE as before (all ranks)
                    if args.target_type != 'class':
                        target_batch = sample_target_from_pool(target_pool, batch_size)
                        mse_to_target = F.mse_loss(backdoor_samples, target_batch)
                        # Optionally: gather MSEs to rank 0 for logging
                        mse_tensor = torch.tensor([mse_to_target.item()], device=device)
                        dist.reduce(mse_tensor, dst=0, op=dist.ReduceOp.SUM)
                        if rank == 0:
                            mse_avg = mse_tensor.item() / dist.get_world_size()
                            logger.info(f'MSE to target: {mse_avg:.4f}')
                    else:
                        if rank == 0:
                            logger.info(f'Using class {args.target_class} as target')
                    
                    # Only rank 0 saves images
                    if rank == 0:
                        from torchvision.utils import save_image
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trigger-type', type=str, default='box', choices=['box', 'fingerprint', 'weight_config', 'pattern', 'custom'], 
